...
Username is used as a part of URI path of account management endpoints. This part of the path always is encoded using \mintinline{js}|encodeURIComponent()| and can be safely sent to the server. Unfortunately, nginx since version 0.8.38 responds with status code 400 if the URI contains a NULL character, even if it is properly encoded. According to Neal Poole the nginx author decided that \enquote{zero byte in URI should not appear in any encoding, operating system, etc., and just makes more problems than helps} and removed support for it in \href{https://hg.nginx.org/nginx/rev/84905c7b2aa7}{changeset 3527} \cite{bib:nginx-null}.
